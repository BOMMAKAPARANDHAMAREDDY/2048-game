const backgroundSound = new Audio("./assets/mixkit-fun-and-games-6.mp3");
...
function placeOnRandomTile() {
  ...
  const block = document.querySelector(
    `.block[data-row="${randomBlock.row}"][data-column="${randomBlock.column}"]`
  );
  ...
  for (let col = 1; col <= 4; col++) {
    for (let row = 2; row <= 4; row++) {
      let currentTile = document.querySelector(
        `.block[data-row="${row}"][data-column="${col}"]`
      );
      if (currentTile.dataset.value != "0") {
        let newRow = row;
        while (newRow > 1) {
          let upTile = document.querySelector(
            `.block[data-row="${newRow - 1}"][data-column="${col}"]`
          );
  ...
  for (let row = 1; row <= 4; row++) {
    for (let col = 2; col <= 4; col++) {
      ...
      if (currentTile.dataset.value != "0") {
        let newCol = col;
        while (newCol > 1) {
          let leftTile = document.querySelector(
            `.block[data-row="${row}"][data-column="${newCol - 1}"]`
          );
  ...
  for (let row = 1; row <= 4; row++) {
    for (let col = 3; col >= 1; col--) {
      ...
      if (currentTile.dataset.value != "0") {
        let newCol = col;
        while (newCol < 4) {
          let rightTile = document.querySelector(
            `.block[data-row="${row}"][data-column="${newCol + 1}"]`
          );
  ...
  for (let col = 1; col <= 4; col++) {
    for (let row = 3; row >= 1; row--) {
      ...
      if (currentTile.dataset.value != "0") {
        let newRow = row;
        while (newRow < 4) {
          let downTile = document.querySelector(
            `.block[data-row="${newRow + 1}"][data-column="${col}"]`
          );
  ...
  for (let row = 1; row <= 4; row++) {
    for (let col = 1; col <= 4; col++) {
      ...
      if (currentTile.dataset.value == "0") {
        ...
        const rightNeighbourTile = document.querySelector(
          `.block[data-row="${row}"][data-column="${col + 1}"]`
        );
        const downNeighbourTile = document.querySelector(
          `.block[data-row="${row + 1}"][data-column="${col}"]`
        );
        if (
          rightNeighbourTile &&
          rightNeighbourTile.dataset.value == currentTile.dataset.value
        ) {
          canMove = true;
          break;
        } else if (
          downNeighbourTile &&
          downNeighbourTile.dataset.value == currentTile.dataset.value
        ) {
          canMove = true;
          break;
        }
